...
$WeekdayActivity = [double]$config["WEEKDAY_ACTIVITY"]
$WeekendActivity = [double]$config["WEEKEND_ACTIVITY"]
...
if ($DateArgs.Count -ne 2) {
    Write-Host "Usage: .\backfill.ps1 [--dry-run] START_DATE END_DATE"
    Write-Host "  Dates in YYYY-MM-DD format"
    Write-Host "  --dry-run   Simulate without committing or pushing"
    Write-Host "  Example: .\backfill.ps1 --dry-run 2025-03-01 2026-02-18"
    exit 1
}

$StartDateStr = $DateArgs[0]
$EndDateStr = $DateArgs[1]
...
if ($DryRun) {
    Write-Host "[DRY RUN] Simulating backfill from $StartDateStr to $EndDateStr..."
} else {
...
while ($CurrentDate -le $EndDate) {
    ...
    if ($CurrentDate.DayOfWeek -in @([DayOfWeek]::Saturday, [DayOfWeek]::Sunday)) {
        $CurrentActivity = $WeekendActivity
    } else {
        $CurrentActivity = $WeekdayActivity
    }
    ...
    if ($DryRun) {
        Write-Host "  $CurrentDateStr - $Target commits (simulated)"
    } else {
...
Write-Host ""
if ($DryRun) {
    Write-Host "[DRY RUN] Simulation complete: $TotalCommits total commits would be created"
} else {
